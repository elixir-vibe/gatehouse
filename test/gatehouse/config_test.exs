defmodule Gatehouse.ConfigTest do
  ...
  test "parses minimal Caddy-like top-level directives" do
    config =
      ...
      acme provider: Gatehouse.ACME.Provider, email: "ops@example.com"
      http port: 80
      https port: 443
    ...
    assert config.state_path == "/var/lib/gatehouse/state.etf"
  ...
  test "builds ACME renewal jobs for auto TLS services" do
    ...
    assert [job] = Gatehouse.ACME.Config.jobs(config)
    assert job.name == "example.com"
    assert job.domains == ["example.com", "www.example.com"]
    assert job.provider == Gatehouse.ACME.Provider.ExAcme
    assert job.store == Gatehouse.CertificateStore.File
    ...
    assert [{:sni_fun, sni_fun}] = Gatehouse.ACME.Config.sni_ssl_opts(config)
  ...
  test "applies active static targets to the runtime control plane" do
    ...
    assert {:ok, "configured", "blue"} = Gatehouse.RouteTable.lookup("configured.example.com")
  end
end
