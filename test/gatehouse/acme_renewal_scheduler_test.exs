defmodule Gatehouse.ACMERenewalSchedulerTest do
  use ExUnit.Case, async: false

  alias Gatehouse.ACME.RenewalScheduler
  alias Gatehouse.CertificateStore.File, as: FileStore
  alias Gatehouse.Test.FakeACMEProvider

  test "renews missing certificates and persists them" do
    directory =
      Path.join(System.tmp_dir!(), "gatehouse-renew-#{System.unique_integer([:positive])}")

    Application.put_env(:gatehouse, :acme_certificates, [
      %{
        name: "example.com",
  ...
  after
    Application.delete_env(:gatehouse, :acme_certificates)
  end

  test "skips certificates that are not due" do
    directory =
      Path.join(System.tmp_dir!(), "gatehouse-skip-#{System.unique_integer([:positive])}")
