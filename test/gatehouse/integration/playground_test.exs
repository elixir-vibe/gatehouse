defmodule Gatehouse.Integration.PlaygroundTest do
  ...
  test "round robin config can require healthy targets" do
    ...
    config = %Gatehouse.Config{
      services: [
        %Gatehouse.Config.Service{
          name: service,
          hosts: [host],
          balance: %{policy: :round_robin, options: [health: :required]},
          targets: [
            %Gatehouse.Config.Target{name: "bad", url: "http://127.0.0.1:9", active?: true},
            %Gatehouse.Config.Target{
              name: "green",
  ...
  test "drain keeps old target while streamed proxied request is active" do
    ...
    assert eventually(fn ->
             {:ok, state} = Gatehouse.Control.get_service(service)
             refute Map.has_key?(state.old_targets, "blue")
           end)
  end

  defp start_livery_listener do
    Gatehouse.LiveryListener.start_link(port: 0, name: unique_name(:livery_listener))
  end
