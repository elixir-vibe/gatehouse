defmodule Gatehouse.LiveryOptionsTest do
  ...
  test "builds HTTP and HTTPS listener options from DSL listener structs" do
    ...
    opts =
      listeners
      |> Gatehouse.LiveryOptions.from_config_listeners()
      |> Enum.reduce(%{handler: &Gatehouse.LiveryHandler.handle/1}, fn listener, acc ->
  ...
  test "adds ACME SNI lookup to HTTPS listeners for auto TLS services" do
    directory =
      Path.join(
        System.tmp_dir!(),
        "gatehouse-livery-acme-sni-#{System.unique_integer([:positive])}"
      )
    ...
    assert [%{scheme: :https, ssl_opts: ssl_opts}] = Gatehouse.LiveryOptions.from_config(config)
  ...
  test "uses TLS certificate file paths when building options" do
    directory =
      Path.join(
        System.tmp_dir!(),
        "gatehouse-livery-options-#{System.unique_integer([:positive])}"
      )
    ...
    assert [opts] = Gatehouse.LiveryOptions.from_config_listeners([listener])
