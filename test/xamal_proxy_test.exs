defmodule XamalProxyTest do
  ...
  test "deploy refuses to switch when health check fails" do
    ...
    assert {:ok, _state} = XamalProxy.Control.deploy(first)
    assert {:error, _reason} = XamalProxy.Control.deploy(bad)
    assert {:ok, "health-fail", "blue"} = XamalProxy.RouteTable.lookup("health.example.com")
  end
  ...
  test "checkin removes drained old targets" do
    ...
    assert {:ok, _target} = XamalProxy.Control.deploy(first)
    assert {:ok, target} = XamalProxy.Control.checkout("drain", "blue")
    assert target.active_requests == 1
    assert {:ok, state} = XamalProxy.Control.deploy(second)
    assert Map.has_key?(state.old_targets, "blue")

    assert :ok = XamalProxy.Control.checkin("drain", "blue")

    assert eventually(fn ->
             {:ok, state} = XamalProxy.Control.get_service("drain")
             refute Map.has_key?(state.old_targets, "blue")
           end)
  end

  test "snapshot can be saved and loaded" do
    path = Path.join(System.tmp_dir!(), "xamal-proxy-#{System.unique_integer([:positive])}.etf")
    ...
    assert {:ok, _state} = XamalProxy.Control.deploy(spec)
    assert :ok = XamalProxy.Control.save(path)
    assert {:ok, %{services: services}} = XamalProxy.Store.load(path)
