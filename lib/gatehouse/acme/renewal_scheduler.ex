defmodule Gatehouse.ACME.RenewalScheduler do
  @moduledoc """
  Periodic ACME certificate renewal scheduler.

  Configure jobs with `:acme_certificates`:

      config :gatehouse, :acme_certificates, [
        %{
          name: "example.com",
          domains: ["example.com"],
          provider: Gatehouse.ACME.Provider.ExAcme,
          provider_opts: [email: "ops@example.com"],
          store: Gatehouse.CertificateStore.File,
          store_opts: [directory: "/var/lib/gatehouse/certs"],
  ...
  defp refresh_tls_listener do
    case Process.whereis(Gatehouse.LiveryListener) do
      nil -> :ok
      _pid -> Gatehouse.LiveryListener.refresh_tls()
    end
  end
  ...
  defp schedule_tick do
    Process.send_after(
      self(),
      :tick,
      Application.get_env(:gatehouse, :acme_renewal_interval, @default_interval)
    )
  end
end
